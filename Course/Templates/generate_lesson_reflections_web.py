...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-pattern",
        default="{source} Reflections.docx",
        help=(
            "Output filename pattern. Available fields: {source} and {docpac}. "
            "Default: {source} Reflections.docx"
        ),
    )
    parser.add_argument(
...
def find_template_parts(document: DocumentObject) -> TemplateParts:
    ...
    for paragraph in document.paragraphs:
        text = clean_text(paragraph.text)
        if text.casefold() == "step reflections":
...
def iter_source_documents(
    ...
    """
    Yield every eligible .docx located DIRECTLY inside docpac_XXXXXX folders.

    The script never recurses into subfolders. The source document's filename
    stem becomes both the worksheet title and the basis of the output filename.
    """
    ...
    for docpac_folder in root.iterdir():
        ...
        # glob("*.docx") checks direct children only; it never enters subfolders.
        for source_path in docpac_folder.glob("*.docx"):
            if not source_path.is_file():
                continue

            lower_name = source_path.name.casefold()

            if lower_name.startswith("docpac_"):
                continue
            if lower_name.startswith("~$"):
                continue
            if lower_name.endswith(" reflections.docx"):
                continue
            if lower_name.endswith("reflections.docx"):
                continue
            if lower_name.endswith(".backup.docx"):
                continue
...
def main() -> int:
    ...
    skipped = 0
    failed = 0
    source_documents = list(iter_source_documents(root))

    if not source_documents:
        print(
            f"No eligible lesson .docx files were found directly inside "
            f"docpac_XXXXXX folders in {root}."
        )
        return 2

    for docpac_code, source_path, source_name in source_documents:
        docpac_folder = source_path.parent
        ...
        try:
            ...
            output_path = docpac_folder / output_name
